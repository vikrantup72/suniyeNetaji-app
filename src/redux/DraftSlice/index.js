...
export const fetchdraftData = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    try {
      const token = await getKey('AuthKey');
      const response = await fetch(
        'https://apis.suniyenetajee.com/api/v1/cms/post/draft',
        {
          method: 'GET',
          headers: {
            Accept: 'application/json',
...
export const deleteDraftPost = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      const token = await getKey('AuthKey');
      const response = await fetch(
        `https://apis.suniyenetajee.com/api/v1/cms/post/delete-draft/${id}/`,
        {
          method: 'DELETE',
          headers: {
            Accept: 'application/json',
...
export const updateDraftPost = createAsyncThunk(
  ...
  async ({ id, updatedData }, { rejectWithValue }) => {
    try {
      ...
      const token = await getKey('AuthKey');
      const response = await fetch(
        `https://apis.suniyenetajee.com/api/v1/cms/post/update-draft/${id}/`,
        {
          method: 'PUT',
          headers: {
            Accept: 'application/json',
